...
class Order(db.Model, UserMixin):
    ...
    def to_dict(self):
        return {
            'id': self.id,
            'name': self.name,
            'order_vaults': [vault.id for vault in self.order_vaults],
            'order_warehouses': [warehouse.id for warehouse in self.order_warehouses],
            'order_fields': [order.id for order in self.order_fields]
        }
